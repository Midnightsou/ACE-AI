// Compresses chat history into a short summary for the system prompt
// We don't send the full history — too many tokens
// We extract the most useful context instead
...
export function buildMemoryContext(profile, recentMessages = []) {
  ...
  if (profile?.examDate) {
    ...
    if (daysLeft > 0) {
      sections.push(`Exam countdown: ${daysLeft} days until their exam`)
    }
  }
  ...
  if (recentMessages.length > 0) {
    ...
    const summary = lastFew
      .map((m) => `${m.role === 'user' ? 'Student' : 'Ace'}: ${m.content.slice(0, 120)}${m.content.length > 120 ? '...' : ''}`)
      .join('\n')
    sections.push(`\nRecent conversation:\n${summary}`)
  }
...
// Detects potential weak areas from the conversation
// Called after each AI response to update the profile
...
function extractTopic(message) {
  const subjects = [
    ...
    'quadratic', 'integration', 'differentiation', 'equations',
